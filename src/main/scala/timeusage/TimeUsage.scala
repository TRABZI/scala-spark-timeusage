...
//    summaryDf.show(3)
    ...
    val typedDs = timeUsageGroupedTyped(summaryDs)
    println("typed..")
    typedDs.show()
  }
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val fields = for (i <- columnNames.indices) yield StructField(columnNames(i), if (i == 0) StringType else DoubleType, nullable = false)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def isPrimary(name: String): Boolean = primaryPrefixes.exists(prefix => name.startsWith(prefix))
    def isWork(name: String): Boolean = workPrefixes.exists(prefix => name.startsWith(prefix))
    def isOther(name: String): Boolean = otherPrefixes.exists(prefix => name.startsWith(prefix))

    def iterate(columns: List[String], primary: List[Column] = List(), work: List[Column] = List(), other: List[Column] = List()): (List[Column], List[Column], List[Column]) = {
      columns.headOption match {
        case Some(name) =>
          name match {
            case n if isPrimary(n) => iterate(columns.tail, col(n) :: primary, work, other)
            case n if isWork(n) => iterate(columns.tail, primary, col(n) :: work, other)
            case n if isOther(n) => iterate(columns.tail, primary, work, col(n) :: other)
            case _ => iterate(columns.tail, primary, work, other)
          }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = (primaryNeedsColumns.reduce(_ + _) / 60D).as("primaryNeeds").cast(DoubleType)
    val workProjection: Column = (workColumns.reduce(_ + _) / 60D).as("work").cast(DoubleType)
    val otherProjection: Column = (otherColumns.reduce(_ + _) / 60D).as("other").cast(DoubleType)
    df
